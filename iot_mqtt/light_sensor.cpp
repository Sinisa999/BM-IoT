...
int main() {
    ...
    while (true) {
        ...
        std::string payload = std::to_string(light);
        mosquitto_publish(mosq, NULL, topic_light, payload.length(), payload.c_str(), 0, false);

        std::cout << "Published light intensity: " << light << " lux\n";
